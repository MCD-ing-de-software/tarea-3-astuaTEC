...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        cleaner = DataCleaner()
        cols = ["name", "age"]
        result = cleaner.drop_invalid_rows(df, cols)
        
        self.assertEqual(result[cols].isna().sum().sum(), 0)
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        # Crear un DataFrame con espacios en blanco usando make_sample_df()
        df = make_sample_df()
        # Rellenar valores nulos para asegurar que la columna sea de tipo string
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        df = make_sample_df()
        cleaner = DataCleaner()
        # Ajustar factor para detectar el outlier en este dataset pequeño
